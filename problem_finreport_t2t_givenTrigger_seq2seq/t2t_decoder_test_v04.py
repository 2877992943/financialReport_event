...
if __name__ == "__main__":
  testll="""TIME , 实 现 triggerRoot mEntity , trigger mEntity 。
TIME , 实 现 trigger mEntity , triggerRoot mEntity 。
TIME 及 TIME , company triggerRoot 分 别 为 mEntity 、 mEntity 、 mEntity 和 mEntity , 占 trigger 的 比 重 分 别 为 mEntity 、 mEntity 、 mEntity 和 mEntity , 占 比 较 低 。
company 的 trigger 主 要 由 triggerRoot 构 成 , TIME TIME , mEntity 合 计 占 比 为 mEntity 、 mEntity 、 mEntity 和 mEntity 。
    """.split('\n')
  testll=[testll[0]]+[testll[3]]
  testll=[line.split(' ') for line in testll]
  orderDict=dict(zip(list(range(len(testll))),list(range(len(testll)))))

  tf.logging.set_verbosity(tf.logging.INFO)
  FLAGS.data_dir = './data'
  FLAGS.t2t_usr_dir = './'
  FLAGS.problem = 'class_seq2seq'
  ##
  FLAGS.model = 'transformer'
  # FLAGS.hparams_set ='transformer_tiny' #transformer_l4
  FLAGS.hparams_set = 'transformer_l2'
  FLAGS.output_dir = './model'
  FLAGS.decode_hparams = "beam_size=4,alpha=0.6"
  FLAGS.decode_from_file='./corpus/tmp.txt'
